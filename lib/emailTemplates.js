...
function applyEmail({
  ...
  // Human-readable work duration label
  const workDurationLabels = {
    temporary: 'Temporary / Contract',
    seasonal:  'Seasonal',
    permanent: 'Permanent',
    flexible:  'Open / Flexible',
  }
  const workDurationLabel = workDurationLabels[workDuration] || null

  const body = `

    ${sectionTitle('Job Application', 'New Application Received')}

    ${infoCard('Applicant Details', `
      ${fieldRow('Full Name',    fullName)}
      ${fieldRow('Email Address', email)}
      ${fieldRow('Phone Number', phone)}
      ${fieldRow('Date of Birth', dob)}
      ${fieldRow('Home Address', address)}
    `)}

    ${infoCard('Position & Availability', `
      ${fieldRow('Position Applied For', position)}
      ${fieldRow('Availability',         availability)}
      ${workDurationLabel ? fieldRow('Work Duration', workDurationLabel) : ''}
      ${additionalInfo    ? fieldRow('Additional Info', additionalInfo)   : ''}
    `)}

    ${message ? messageBlock('Message / Cover Note', escapeHtml(message)) : ''}
    ...
    ${ctaButton(`mailto:${escapeHtml(email)}`, `Reply to ${escapeHtml(fullName)}`)}
  `

  return {
    html:    wrap('New Job Application — Bluvora Resources', body),
    subject: `[Application] ${fullName} — ${position}`,
  }
}

module.exports = { contactEmail, resumeEmail, applyEmail }
